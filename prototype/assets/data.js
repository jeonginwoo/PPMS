...
(function (g) {
  ...
  /* 휴가 — 나중에 팀즈 휴가 정보를 연동해 채울 자리다(사용자 방향, 2026-09-22).
     지금은 목업이고 **일간 보고에서 표시만** 한다. 연동도, "자동 처리"도 만들지 않았다 —
     자동 처리가 ① 그 날 TODO를 안 만드는 것인지 ② 있는 것을 완료로 치는 것인지
     ③ 다음 근무일로 넘기는 것인지가 안 정해졌고, 그건 이 화면이 아니라 그 단위가 답할 것이다.

     일수 환산(연차 1 · 반차 0.5 · 반반차 0.25)은 2026-09-23에 넣었다가 **주간보고에서 휴가를
     빼기로 하면서 쓰는 곳이 없어져 지웠다.** 환산 자체는 F12·가동률(F3)에서 다시 필요해질 값이고,
     그때 되살리면 된다 — 쓰지 않는 코드를 미리 들고 있지 않는다. */
  var leaves = [
    { personId: 2, date: T0, type: '반차' },
    { personId: 3, date: T1, type: '연차' }
  ];
